...
Experiment::Run (uint32_t nNodes)
{
  ...
  mob->AddWaypoint (Waypoint (Seconds (0.0), Vector (0,0,100)));
  mob->AddWaypoint (Waypoint (Seconds (43200.0), Vector (5,0,0)));
  mob->AddWaypoint (Waypoint (Seconds (86400.0), Vector (10,0,100)));
  ...
  internetStackHelper.Install (auv); // AUV
  ...
  Ipv4InterfaceContainer interfaces_auv = ipv4AddressHelper.Assign (auvDev); //AUV
  ...
  /**********************SEND PACKETS********************************/
   ...
   //Ptr<Packet> pkt = Create<Packet> (); // ns3 standard: 32 bits
   ...
   //NodeContainer::Iterator inode = sinks.Begin ();
   ...
   while (inode_pkt != nodes.End()){
	   ...
	   while(isink_pkt != sinks.End()){
         ...
         while(inter_arrival_time <= m_simTime.GetDouble()/1e9){
			   		    	  ...
			   		    	  //std::cout << "value 2:" <<  inter_arrival_time << std::endl;
 ...
 // Flow monitor
  ...
  dl_results = m_outputdir + "/" + "DL_Results_Sim_PropModel_"+ m_propModel+"_"+"MaxRange_"+sMaxRange+"_"+"AuvModel_"+sAuvModel +".txt";
  ul_results = m_outputdir + "/" + "UL_Results_Sim_PropModel_"+ m_propModel+"_"+"MaxRange_"+sMaxRange+"_"+"AuvModel_"+sAuvModel +".txt";
 ...
 Ptr<Ipv4FlowClassifier> classifier = DynamicCast<Ipv4FlowClassifier>(flowHelper.GetClassifier());
  ...
  flowMonitor->SerializeToXmlFile("Sim_PropModel_"+ m_propModel+"_"+"MaxRange_"+sMaxRange +".xml" , true, true);
...
static ns3::GlobalValue g_propModel("PropModel",
                              "Propagation Model",
                              ns3::StringValue ("UanPropModelThorp"),
                              ns3::MakeStringChecker());
...
//TODO: Criar para o modelo do AUV
  ...
  uint32_t packetsRx = exp.Run (m_numNodes);
